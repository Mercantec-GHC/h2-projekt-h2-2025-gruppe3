...
namespace Blazor.Services
{
	public partial class APIService
	{
		...
		public async Task<bool> RegisterAsync(RegisterDto registerDto)
		{
			try
			{
				var response = await httpClient.PostAsJsonAsync("api/users/register", registerDto);
		...
		public async Task<UserGetDto[]?> GetUsersAsync()
		{
			try
			{
				return await httpClient.GetFromJsonAsync<UserGetDto[]>("api/users");
			}
			catch (Exception ex)
			{
		...
		public async Task<UserGetDto?> GetUserAsync(string id)
		{
			try
			{
				return await httpClient.GetFromJsonAsync<UserGetDto>($"api/users/{id}");
			}
			catch (Exception ex)
			{
		...
		public async Task<UserGetDto?> GetCurrentUserAsync()
		{
			try
			{
				return await httpClient.GetFromJsonAsync<UserGetDto>("api/users/me");
			}
			catch (Exception ex)
			{
